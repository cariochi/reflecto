...
public class TypesUtils {
    ...
    public static Type getActualType(Type type, Type assigningType) {
        if (type instanceof Class<?>) {
            ...
            if (hasTypeVariables) {
                return recognizeTypeArguments(parameterizedType, assigningType);
            } else {
                return type;
            }
        } else if (type instanceof TypeVariable<?>) {
            return assigningType == null ? null : recognizeTypeVariable((TypeVariable<?>) type, assigningType);
        } else if (type instanceof GenericArrayType) {
            return recognizeGenericArrayType((GenericArrayType) type, assigningType);
        } else {
            return type;
        }
    }

    private static Type recognizeTypeArguments(ParameterizedType type, Type assigningType) {
    ...
    private static Type recognizeTypeVariable(TypeVariable<?> type, Type assigningType) {
        ...
        return Optional.ofNullable(TypeUtils.getTypeArguments(assigningType, aClass))
                .or(() -> Optional.ofNullable(TypeUtils.determineTypeArguments(aClass, (ParameterizedType) assigningType)))
                .map(m -> m.get(type))
                .map(typeArgument -> getActualType(typeArgument, assigningType))
                .orElse(type);
    }

    private static Type recognizeGenericArrayType(GenericArrayType type, Type assigningType) {
